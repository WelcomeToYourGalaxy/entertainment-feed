#!/usr/bin/env python3
"""
harvest_entertainment.py — the entertainment wire: who controls the bottleneck,
what decides which stories reach people, how much of their time it takes, and
how far into them the measurement now reaches.

After "The Entertainment Industries", which frames it as a single continuous
question — whoever controls the bottleneck controls what stories populations
consume, whether that is Roman emperors, medieval churches, Hollywood studios,
broadcast networks or platform algorithms. It puts numbers to the modern
version: six studios taking most of one continent's box office, three companies
distributing most of its recorded music, six accounting for more than half the
time spent watching television. Then the internet moved the bottleneck into
search ranking, recommendation engines and feeds, with viewer behaviour tracked
individually.

Then the part the section is most concerned with: measurement reaching inward.
Consumer neurotechnology and emotion sensing that read neural and emotional
states, register microexpressions and voice inflection, and attempt to infer
intent before a person acts — with the resulting biometric and behavioural data
flowing onward. Around that: the hours taken, children's share of them, the
adult industry and its ownership, the people who make the work and what they
are paid, synthetic performers and likeness, game monetisation, moderation,
state control, local production, rights, and engagement design.

A review, a trailer, a casting announcement or a box-office weekend is not this
subject. Every term carries the industry or control words it must appear beside.

    python3 harvest_entertainment.py
    python3 harvest_entertainment.py --dry-run
"""
...
# --------------------------------------------------------------------------
USER_AGENT = ("Mozilla/5.0 (compatible; entertainment-feed/1.0; "
...
# THE SUBJECTS
#
# Who controls the bottleneck, what decides which stories reach people, how
# much of their time it takes, and how far into them the measurement reaches.
#
# This is a feed on the industry, not on its output. A review, a trailer, a
# casting announcement and a box-office weekend are refused, and every term
# carries the industry or control words it must appear beside.
# --------------------------------------------------------------------------
TOPICS = [
    ("bottlenecks", "Control of the bottleneck", [
        ("distribution deal", ["exclusive", "control", "terms", "blocked", "dispute"]),
        ("gatekeep*", ["platform", "distribution", "industry", "studio", "algorithm"]),
        ("vertical integration", []), ("exhibition", ["control", "chain", "monopol*", "ruling"]),
        ("theatrical window", []), ("release window", []),
        ("carriage dispute", []), ("must-carry", []),
        ("bundling", ["channel", "streaming", "distribution", "forced"]),
        ("patent", ["film", "projection", "distribution", "control", "trust"]),
        ("monopol*", ["studio", "distribution", "streaming", "music", "cinema", "platform"]),
    ]),
    ("studios", "Film and the studios", [
        ("box office", ["share", "concentration", "dominat*", "studios", "per cent"]),
        ("studio", ["merger", "acquisition", "consolidat*", "market share", "layoffs", "shut"]),
        ("paramount", ["merger", "sale", "acquisition", "layoffs", "studio"]),
        ("warner bros", ["merger", "sale", "acquisition", "layoffs", "split"]),
        ("disney", ["merger", "acquisition", "layoffs", "market share", "antitrust", "consolidat*"]),
        ("universal", ["merger", "acquisition", "market share", "studio"]),
        ("independent film", ["funding", "squeeze", "distribution", "decline", "shut"]),
        ("film fund", ["cut", "closed", "criteria", "allocation"]),
    ]),
    ("music", "Recorded music", [
        ("record label", ["merger", "acquisition", "market share", "dominat*", "contract", "dispute"]),
        ("major label", []), ("universal music", []), ("sony music", []), ("warner music", []),
        ("streaming royalt*", []), ("royalty rate", []), ("per-stream", []),
        ("catalogue sale", []), ("catalog sale", []), ("music rights", ["sold", "acquired", "fund", "portfolio"]),
        ("artist contract", []), ("recoupment", []), ("360 deal", []),
        ("payola", []), ("playlist", ["pay", "placement", "influence", "manipulat*", "gaming"]),
        ("stream farming", []), ("fake streams", []), ("streaming fraud", []),
    ]),
    ("television", "Television and streaming", [
        ("streaming service", ["price", "merger", "losses", "subscribers", "consolidat*", "bundle"]),
        ("netflix", ["price", "subscribers", "policy", "data", "market share", "antitrust"]),
        ("watch time", ["share", "concentration", "companies", "dominat*"]),
        ("viewing time", ["share", "concentration", "companies"]),
        ("cord cutting", []), ("channel closure", []), ("linear tv", ["decline", "share", "collapse"]),
        ("password sharing", []), ("ad tier", []), ("price rise", ["streaming", "subscription", "service"]),
        ("content spend", ["cut", "reduced", "budget", "slashed"]),
        ("shelved", ["film", "series", "show", "tax write-off"]), ("tax write-off", []),
    ]),
    ("algorithms", "What decides who sees what", [
        ("recommendation algorithm", []), ("recommendation engine", []),
        ("algorithmic curation", []), ("algorithmic feed", []),
        ("ranking", ["algorithm", "changed", "downranked", "boost*", "suppress*", "creators"]),
        ("discoverability", []), ("reach", ["algorithm", "collapsed", "throttled", "creators", "suppress*"]),
        ("autoplay", ["next", "default", "design", "removed"]),
        ("watch next", []), ("rabbit hole", ["recommendation", "algorithm", "study", "radicalis*"]),
        ("audit", ["algorithm", "recommendation", "researchers", "access"]),
        ("researcher access", ["platform", "data", "denied", "api"]),
    ]),
    ("tracking", "Measuring the viewer", [
        ("viewer data", []), ("viewing data", ["collected", "sold", "shared", "tracked"]),
        ("behavioural data", ["viewer", "user", "collected", "sold", "profiling"]),
        ("behavioral data", ["viewer", "user", "collected", "sold", "profiling"]),
        ("smart tv", ["tracking", "data", "acr", "privacy", "collect*"]),
        ("automatic content recognition", []), ("acr", ["smart tv", "tracking", "data"]),
        ("personalis*", ["feed", "recommendation", "advert*", "content", "at scale"]),
        ("personaliz*", ["feed", "recommendation", "advert*", "content", "at scale"]),
        ("profiling", ["viewer", "user", "audience", "banned", "ruling"]),
        ("a/b test", ["users", "emotional", "without consent", "platform"]),
    ]),
    ("neural", "Reading the body and the mind", [
        ("brain-computer interface", []), ("neurotechnolog*", []), ("neural data", []),
        ("neuralink", []), ("eeg", ["headset", "consumer", "advert*", "measure", "wearable"]),
        ("neurorights", []), ("mental privacy", []), ("cognitive libert*", []),
        ("emotion recognition", []), ("emotion detection", []), ("affective computing", []),
        ("affectiva", []), ("smart eye", ["emotion", "eye tracking", "measure"]),
        ("microexpression*", []), ("voice inflection", ["analysis", "measure", "detect"]),
        ("eye tracking", []), ("intent", ["predict*", "detect*", "before they act", "inferred"]),
        ("neuromarketing", []), ("neural signal*", ["advert*", "campaign", "measure", "response"]),
    ]),
    ("biometrics", "Biometric data and where it goes", [
        ("biometric data", []), ("biometric", ["consent", "collected", "sold", "ruling", "law", "fine"]),
        ("facial recognition", ["venue", "cinema", "stadium", "audience", "banned", "deployed"]),
        ("bipa", []), ("biometric privacy", []),
        ("data broker", ["viewer", "audience", "behavioural", "behavioral"]),
        ("surveillance", ["audience", "viewer", "consumer", "entertainment", "venue"]),
        ("consent", ["biometric", "neural", "emotional", "data", "withdrawn", "invalid"]),
        ("wearable", ["data", "collected", "shared", "health", "sold"]),
    ]),
    ("screentime", "Hours taken", [
        ("screen time", []), ("hours per day", ["media", "screen", "watching", "average"]),
        ("media consumption", ["hours", "average", "rose", "survey", "study"]),
        ("time spent", ["platform", "app", "media", "watching", "rose", "average"]),
        ("attention economy", []), ("engagement metric*", []),
        ("daily active", ["users", "engagement", "time"]),
        ("compuls*", ["use", "viewing", "scrolling", "design"]),
        ("digital wellbeing", []), ("time limit*", ["app", "platform", "screen", "default"]),
    ]),
    ("children", "Children and young audiences", [
        ("children", ["screen time", "advert*", "platform", "age verification", "design code",
                      "harm", "regulat*", "data", "targeted"]),
        ("minors", ["platform", "age", "protection", "data", "banned", "curfew"]),
        ("age verification", []), ("age assurance", []), ("age-appropriate design", []),
        ("children's code", []), ("kidfluencer", []), ("child performer", []),
        ("coogan", []), ("social media ban", ["under", "minors", "age", "teen"]),
        ("teen", ["mental health", "platform", "design", "harm", "lawsuit"]),
    ]),
    ("adult", "The adult industry", [
        ("adult industry", []), ("adult platform", []), ("pornograph*", ["industry", "platform",
                                 "regulat*", "age verification", "law", "ban", "share of traffic",
                                 "owner", "consent", "takedown", "payment"]),
        ("mindgeek", []), ("aylo", []), ("onlyfans", []),
        ("age verification", ["adult", "porn", "site", "law"]),
        ("non-consensual", ["image", "intimate", "material", "takedown", "law"]),
        ("intimate image", ["non-consensual", "takedown", "law", "removal"]),
        ("payment processor", ["adult", "platform", "cut off", "policy", "pressure"]),
        ("content moderation", ["adult", "platform", "verification", "failure"]),
        ("performer", ["consent", "contract", "rights", "conditions", "union", "pay"]),
    ]),
    ("labour", "Who makes it and what they are paid", [
        ("residual*", []), ("writers strike", []), ("actors strike", []), ("wga", []), ("sag-aftra", []),
        ("strike", ["writers", "actors", "crew", "animators", "game workers", "studio"]),
        ("union", ["recognition", "crew", "writers", "actors", "game", "vfx", "contract"]),
        ("crew", ["conditions", "hours", "pay", "safety", "walkout"]),
        ("vfx", ["conditions", "pay", "union", "outsourc*", "crunch"]),
        ("crunch", ["studio", "developers", "overtime", "culture"]),
        ("freelance", ["rates", "pay", "conditions", "precarious"]),
        ("layoff*", ["studio", "streaming", "games", "animation", "publisher"]),
    ]),
    ("ai", "Synthetic performers and likeness", [
        ("synthetic performer", []), ("digital replica", []), ("digital double", []),
        ("likeness", ["rights", "consent", "used", "scanned", "protection", "law"]),
        ("voice clone", []), ("voice actor", ["ai", "consent", "replaced", "strike", "contract"]),
        ("deepfake", ["performer", "actor", "likeness", "consent", "law", "removed"]),
        ("ai-generated", ["film", "music", "show", "character", "script", "disclosure"]),
        ("generative ai", ["studio", "production", "music", "replace", "contract", "policy"]),
        ("training data", ["music", "film", "scripts", "consent", "licensed", "lawsuit"]),
        ("scanned", ["extras", "background actors", "consent", "likeness"]),
    ]),
    ("games", "Games and how they are monetised", [
        ("loot box", []), ("gacha", []), ("microtransaction*", []),
        ("in-game purchase", []), ("battle pass", []), ("virtual currency", ["regulat*", "refund", "conversion"]),
        ("game monetis*", []), ("game monetiz*", []),
        ("addictive design", ["game", "app", "platform"]),
        ("player spending", []), ("whale", ["spending", "players", "revenue"]),
        ("refund", ["game", "in-app", "regulator", "ordered"]),
        ("game workers", []), ("studio closure", ["game", "development"]),
    ]),
    ("moderation", "What is allowed through", [
        ("content moderation", []), ("takedown", ["content", "video", "channel", "order", "request"]),
        ("demonetis*", []), ("demonetiz*", []),
        ("banned", ["channel", "creator", "account", "video", "platform"]),
        ("community guidelines", []), ("appeal", ["removal", "moderation", "ban", "creator"]),
        ("moderator", ["conditions", "trauma", "outsourc*", "pay", "union"]),
        ("shadow ban*", []), ("age rating", ["refused", "changed", "board", "banned"]),
        ("classification board", []), ("censor*", ["film", "series", "game", "scene", "cut", "banned"]),
    ]),
    ("statecontrol", "State control of the story", [
        ("state broadcaster", ["control", "board", "funding", "independence", "appointed"]),
        ("propaganda", ["film", "entertainment", "series", "state", "funded"]),
        ("banned in", ["film", "series", "game", "song", "country"]),
        ("cultural ministry", []), ("film board", ["approval", "refused", "cut", "certificate"]),
        ("certificate refused", []), ("licence refused", ["film", "broadcast", "release"]),
        ("soft power", ["cultural", "film", "entertainment", "funding", "strategy"]),
        ("script approval", []), ("co-production", ["approval", "conditions", "censor*", "market access"]),
        ("market access", ["film", "studio", "conditions", "censor*", "cuts"]),
    ]),
    ("localculture", "Local production and what displaces it", [
        ("local content", ["quota", "requirement", "investment", "obligation"]),
        ("content quota", []), ("cultural exception", []), ("cultural policy", []),
        ("local production", ["decline", "funding", "quota", "displaced", "investment"]),
        ("national cinema", []), ("dubbing", ["industry", "ai", "jobs", "replaced"]),
        ("subtitle", ["industry", "ai", "quality", "workers"]),
        ("streaming levy", []), ("investment obligation", []),
        ("homogenis*", ["culture", "content", "narrative", "output"]),
        ("homogeniz*", ["culture", "content", "narrative", "output"]),
    ]),
    ("rights", "Who owns the work", [
        ("copyright", ["ruling", "lawsuit", "reform", "term", "infringement", "training"]),
        ("piracy", ["site", "blocking", "enforcement", "ruling", "losses"]),
        ("rights reversion", []), ("orphan work*", []), ("public domain", ["entered", "work", "year"]),
        ("licensing dispute", []), ("music licensing", []), ("sync licence", []),
        ("collecting societ*", []), ("performance rights", []),
        ("class action", ["artists", "authors", "musicians", "training", "likeness", "royalt*"]),
        ("lawsuit", ["copyright", "training data", "likeness", "royalt*", "artists", "authors"]),
    ]),
    ("dependency", "Engagement, design and dependency", [
        ("engagement", ["optimis*", "optimiz*", "maximis*", "maximiz*", "design", "harm", "metric"]),
        ("dopamine", ["design", "loop", "engineered"]), ("variable reward", []),
        ("infinite scroll", []), ("streak", ["design", "app", "engagement"]),
        ("notification", ["design", "engagement", "volume", "default"]),
        ("dark pattern", []), ("cancellation", ["difficult", "dark pattern", "ruling", "click to cancel"]),
        ("subscription trap", []), ("auto-renew", ["ruling", "rules", "refund"]),
        ("internal research", ["harm", "platform", "leaked", "knew"]),
    ]),
    ("resistance", "What is set against it", [
        ("antitrust", ["studio", "streaming", "music", "platform", "ticketing", "case", "remedy"]),
        ("ticketmaster", []), ("live nation", []),
        ("regulator", ["fined", "ruling", "investigation", "streaming", "platform", "games"]),
        ("class action", ["streaming", "platform", "subscribers", "gamers", "consumers"]),
        ("right to repair", ["game", "console", "software"]), ("preservation", ["game", "film", "archive", "lost"]),
        ("public broadcaster", ["funding", "independence", "remit"]),
        ("cooperative", ["artist", "creator", "owned", "platform"]),
        ("campaign", ["screen time", "age verification", "loot box", "residuals", "likeness"]),
        ("ruling", ["loot box", "likeness", "residual*", "streaming", "copyright"]),
    ]),
]

ANCHOR = [
    # Entertainment-industry-control language. A review or a trailer is not this
    # subject; who controls the bottleneck, what decides who sees what, how much
    # time it takes and how deep the measurement goes, is.
    "recommendation algorithm", "recommendation engine", "algorithmic curation",
    "discoverability", "watch time share", "viewing time share",
    "box office share", "market share", "vertical integration", "carriage dispute",
    "streaming royalties", "royalty rate", "per-stream", "payola", "streaming fraud",
    "residuals", "writers strike", "actors strike", "sag-aftra", "wga",
    "synthetic performer", "digital replica", "likeness rights", "voice clone",
    "loot box", "microtransactions", "gacha", "battle pass", "game monetisation",
    "content moderation", "demonetised", "demonetized", "shadow ban",
    "age verification", "age assurance", "age-appropriate design", "children's code",
    "screen time", "attention economy", "engagement optimisation", "dark pattern",
    "brain-computer interface", "neurotechnology", "neural data", "neurorights",
    "emotion recognition", "affective computing", "neuromarketing", "eye tracking",
    "biometric data", "biometric privacy", "automatic content recognition",
    "smart tv tracking", "viewer data", "data broker",
    "content quota", "local content", "cultural exception", "streaming levy",
    "classification board", "film board", "certificate refused",
    "antitrust", "ticketmaster", "live nation", "copyright ruling", "piracy blocking",
    "mindgeek", "aylo", "onlyfans", "non-consensual intimate images",
]

BLOCK = [
    # ordinary entertainment coverage, which is nearly all entertainment coverage
    "film review", "movie review", "tv review", "series review", "album review",
    "game review", "first look", "trailer", "teaser", "clip", "behind the scenes",
    "casting", "joins the cast", "signs on to star", "set photos", "premiere",
    "red carpet", "awards season", "oscars", "grammys", "emmys", "golden globes",
    "box office weekend", "opening weekend", "tops the chart", "number one single",
    "release date", "streaming this month", "what to watch", "best films of",
    "recap", "episode", "season finale", "spoilers", "easter egg", "fan theory",
    "interview with", "talks about", "opens up about", "reveals", "teases",
    "celebrity", "dating", "split", "engagement ring", "net worth",
    "tour dates", "setlist", "festival lineup", "concert review",
    # unrelated filler
    "recipe", "gift guide", "coupon", "black friday", "horoscope",
    "sponsored content", "partner content", "transfer rumour",
]
...
DECIDED = [
    ...
    "fined", "settled", "blocked", "ordered", "divest", "convicted", "withdrawn",
    "certificate refused", "licence refused", "complaint upheld",
]
INSTITUTIONAL = [
    "european commission", "federal trade commission", "department of justice",
    "competition and markets authority", "ofcom", "information commissioner",
    "data protection authority", "cnil", "european parliament", "unesco",
    "classification board", "film board", "regulator", "auditor general",
    "national audit office", "government accountability office",
    "parliamentary committee", "select committee", "official gazette",
    "court filing", "inspector general",
    "peer-reviewed", "published in", "study finds", "working paper", "dataset",
    "official data", "government figures", "national statistics", "nielsen",
]
MEASURED = [
    "per cent", "percent", "%", "million", "billion", "hours", "minutes",
    "figures show", "fell by", "rose by", "increase of", "decrease of",
    "estimated", "median", "average", "ranked", "index", "share of", "rate of",
    "subscribers", "streams", "viewers", "market share", "watch time",
]
PENDING = [
...
def run(dry_run=False, fixtures=None):
    ...
    payload = {
        ...
        "languages": languages,
        "standings": [
            {"id": "official", "label": "Regulators, courts & classification bodies"},
            {"id": "research", "label": "Research & industry analysis"},
            {"id": "press", "label": "Press"},
            {"id": "rights", "label": "Unions, privacy & audience groups"},
        ],
    ...
    if dry_run:
        print("\n--dry-run: wire_entertainment.json not written")
        return payload
